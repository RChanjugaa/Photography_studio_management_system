...
Module / Page: Booking & Service Management\ Theme: ShootProof inspired modern SaaS (clean neutrals + soft pastel accents)\ Primary Responsibilities: Create/manage bookings, calendar & availability, package (service) CRUD, status transitions (pending → confirmed → completed/cancelled), reschedule, cancellations.\ Explicit Boundaries:
